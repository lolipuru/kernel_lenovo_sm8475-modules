...
//static int cs35l45_reset_flag = 0;
...
static const char * const gain_texts[] = {"dB_10", "dB_13", "dB_16", "dB_19"};
...
static int cs35l45_dai_set_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	printk("cs35l45_dai_set_fmt enter \n");
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		...
		break;
	default:
		dev_err(cs35l45->dev, "Mixed master mode unsupported (%d)\n",
	...
	case SND_SOC_DAIFMT_I2S:
		...
		break;
	default:
		dev_err(cs35l45->dev, "Unsupported DAI format (%d)\n",
	...
	case SND_SOC_DAIFMT_NB_NF:
		...
		break;
	default:
		dev_err(cs35l45->dev, "Invalid clock polarity (%d)\n",
				 ...
				 struct snd_soc_dai *dai)
{
	...
	printk("cs35l45_dai_hw_params enter\n");
...
static irqreturn_t cs35l45_spk_safe_err(int irq, void *data)
{
	...
	size = ARRAY_SIZE(cs35l45_irqs);

	i = irq - regmap_irq_get_virq(cs35l45->irq_data, 0);

	if (i < 0 && i >= size) {
		dev_err(cs35l45->dev, "BigElt: %s : array index out of bounds!\n", __func__);
		return IRQ_HANDLED;
	}
...
int cs35l45_probe(struct cs35l45_private *cs35l45)
{
	...
	if (IS_ERR(cs35l45->reset_gpio)) {
                dev_err(dev, "Failed to get reset GPIO error : line %d\n", __LINE__);
	...
	if (cs35l45->reset_gpio) {
                dev_err(dev, "get reset GPIO ok1: line %d\n", __LINE__);
		gpiod_set_value_cansleep(cs35l45->reset_gpio, 0);
                dev_err(dev, "get reset GPIO ok2: line %d\n", __LINE__);
		usleep_range(2000, 4000);
                dev_err(dev, "get reset GPIO ok3: line %d\n", __LINE__);
		gpiod_set_value_cansleep(cs35l45->reset_gpio, 1);
                usleep_range(2000, 4000);
                dev_err(dev, "get reset GPIO ok4: line %d\n", __LINE__);
	}

	/*if(cs35l45_reset_flag == 0) {
		cs35l45_reset_flag = 1;

		cs35l45->pinctrl = devm_pinctrl_get(dev);
		if (IS_ERR_OR_NULL(cs35l45->pinctrl)) {
			dev_err(dev, "%s: Cannot get cs35l45 gpio pinctrl:%ld\n",
				__func__, PTR_ERR(cs35l45->pinctrl));
			goto err;
		}

		cs35l45->reset_pinctrl_active = pinctrl_lookup_state(
					cs35l45->pinctrl, "cs35l45_reset_active");
		if (IS_ERR_OR_NULL(cs35l45->reset_pinctrl_active)) {
			dev_err(dev, "%s: Cannot get aud_active pinctrl state:%ld\n",
				__func__, PTR_ERR(cs35l45->reset_pinctrl_active));
			goto err;
		}

		cs35l45->reset_pinctrl_sleep = pinctrl_lookup_state(
					cs35l45->pinctrl, "cs35l45_reset_sleep");
		if (IS_ERR_OR_NULL(cs35l45->reset_pinctrl_sleep)) {
			dev_err(dev, "%s: Cannot get aud_active pinctrl state:%ld\n",
				__func__, PTR_ERR(cs35l45->reset_pinctrl_sleep));
			goto err;
		}
		printk("cs35l45 reset \n");
		ret = pinctrl_select_state(cs35l45->pinctrl, cs35l45->reset_pinctrl_active);
		if (ret) {
			dev_err(dev, "state for reset_pinctrl_active, err=%d\n",ret);
			goto err;
		}
		usleep_range(2000,3000);
		ret = pinctrl_select_state(cs35l45->pinctrl, cs35l45->reset_pinctrl_sleep);
		if (ret) {
			dev_err(dev, "state for reset_pinctrl_sleep, err=%d\n",ret);
			goto err;
		}
